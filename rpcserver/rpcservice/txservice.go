...
func (txService TxService) ChooseOutsCoinByKeysetForEstimateFee(
	...
	candidateOutputCoins, remainOutCoins, totalCandidateOutCoinsAmount, err := txService.chooseBestOutCoinsToSpent(outCoins, totalAmmount)
	if err != nil {
		return nil, nil, 0, 0, NewRPCError(GetOutputCoinError, err)
	}

	overBalance := totalCandidateOutCoinsAmount - totalAmmount
